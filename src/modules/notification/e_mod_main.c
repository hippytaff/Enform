                          ...
                          int         replaces_id)
{
   E_Notification *n = e_notification_full_new
       ("enform", replaces_id, "enform", summary, body, -1);
   ...
   if (enabled)
     {
        ...
        body = _("Enform is in <b>presentation</b> mode."
   ...
   if (enabled)
     {
        ...
        body = _("Enform is in <b>offline</b> mode.<br>"
...
#define D conf_edd
   ...
   d = e_notification_daemon_add("e_notification_module", "Enform");
   if (!d)
     {
        _notification_cfg_free(notification_cfg);
        notification_cfg = NULL;
