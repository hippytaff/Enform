...
_ilist_region_cb_change(void *data, Evas_Object *obj __UNUSED__)
{
   ...
   if ((cfdata->cur_lang) && (cfdata->cur_lang[0]))
     {
        snprintf(locale, sizeof(locale),  "%s_%s", cfdata->cur_lang, cfdata->cur_reg);
...
_ilist_codeset_cb_change(void *data, Evas_Object *obj __UNUSED__)
{
   ...
   if ((cfdata->cur_lang) && (cfdata->cur_lang[0]))
     {
        if (cfdata->cur_mod)
          snprintf(locale, sizeof(locale), "%s_%s.%s@%s", cfdata->cur_lang, cfdata->cur_reg, cfdata->cur_cs, cfdata->cur_mod);
        else
          snprintf(locale, sizeof(locale), "%s_%s.%s", cfdata->cur_lang, cfdata->cur_reg, cfdata->cur_cs);
        e_widget_entry_text_set(cfdata->gui.locale_entry, locale);
     }
   else
     e_widget_entry_text_set(cfdata->gui.locale_entry, "");
   ...
   if ((cfdata->cur_lang) && (cfdata->cur_lang[0]))
     {
        if (cfdata->cur_cs)
          ...
          snprintf(locale, sizeof(locale), "%s_%s@%s", cfdata->cur_lang, cfdata->cur_reg, cfdata->cur_mod);
        e_widget_entry_text_set(cfdata->gui.locale_entry, locale);
     }
   else
     e_widget_entry_text_set(cfdata->gui.locale_entry, "");
