   ...
   if (ret != 0)
     {
        ...
        if (!(dia = e_dialog_new(con, "E", "_e_mod_cpufreq_error_setfreq")))
          return;
        e_dialog_title_set(dia, "Enform Cpufreq Module");
        e_dialog_icon_set(dia, "enform", 64);
...
{
/* BOGO, everyone else needs a buffty */
#ifndef __OpenBSD__
   char buf[4096];
   Eina_List *l;
        ...
        do
          {
             while ((*gov) && (isspace(*gov)))
               gov++;
             ssize_t len = strlen(gov);
             if (gov != 0) { /* BOGO below fixes a bug last char is \n*/
            	if (gov[len - 1] == '\n')
                   gov[len - 1] = '\0';     
