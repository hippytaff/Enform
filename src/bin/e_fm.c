...
_e_fm2_client_spawn(void)
{
   ...
   snprintf(buf, sizeof(buf), "%s/enform/utils/enform_fm", e_prefix_lib_get());
   ...
   if (desktop->x)
     {
        ...
        type = eina_hash_find(desktop->x, "X-Enform-Type");
        if (type)
          {
             ...
             type = eina_hash_find(desktop->x, "X-Enform-Removable-State");
             if (type)
               {
                  if (!strcmp(type, "Full"))
...
_clear_background_cb(void *data, E_Menu *m __UNUSED__, E_Menu_Item *mi __UNUSED__)
{
   ...
   _custom_file_key_del(sd, "X-Enform-Directory-Wallpaper");
...
_clear_overlay_cb(void *data, E_Menu *m __UNUSED__, E_Menu_Item *mi __UNUSED__)
{
   ...
   _custom_file_key_del(sd, "X-Enform-Directory-Overlay");
   evas_object_smart_callback_call(sd->obj, "dir_changed", NULL);
}
...
_e_fm2_view_menu_pre(void *data, E_Menu *subm)
{
   ...
   while (sd->desktop)
     {
        if (!eina_hash_find(sd->desktop->x, "X-Enform-Directory-Wallpaper")) break;
        mi = e_menu_item_new(subm);
   ...
   if (!sd->desktop) return;
   if (!eina_hash_find(sd->desktop->x, "X-Enform-Directory-Overlay")) return;
   mi = e_menu_item_new(subm);
...
_set_background_cb(void *data, E_Dialog *dia)
{
   ...
   if (file)
     _custom_file_key_set(sd, "X-Enform-Directory-Wallpaper", file);
...
_set_overlay_cb(void *data, E_Dialog *dia)
{
   ...
   if (file)
     _custom_file_key_set(sd, "X-Enform-Directory-Overlay", file);
