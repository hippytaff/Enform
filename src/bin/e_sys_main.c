...
static int auth_action_ok(char *a,
                          ...
                          int gn,
                          gid_t egid);
static int auth_etc_enform_sysactions(char *a,
                                             char *u,
                                             char **g);
static void auth_etc_enform_sysactions_perm(char *path);
               ...
               gid_t egid)
{
   ...
   grp[j] = NULL;
   /* first stage - check:
    * PREFIX/etc/enform/sysactions.conf
    */
   ret = auth_etc_enform_sysactions(a, usr, grp);
   if (ret == 1) return 1;
   else if (ret == -1)
     return 0;
   /* the DEFAULT - allow */
...
static int
auth_etc_enform_sysactions(char *a,
                                  char *u,
                                  char **g)
{
   FILE *f;
   ...
   snprintf(file, sizeof(file), "/etc/enform/sysactions.conf");
   f = fopen(file, "r");
   if (!f)
     {
        snprintf(file, sizeof(file), PACKAGE_SYSCONF_DIR "/enform/sysactions.conf");
        f = fopen(file, "r");
        if (!f) return 0;
     }

   auth_etc_enform_sysactions_perm(file);
...
static void
auth_etc_enform_sysactions_perm(char *path)
{
   struct stat st;
   if (stat(path, &st) == -1)
     return;
