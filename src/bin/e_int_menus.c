...
e_int_menus_main_new(void)
{
   ...
   l = _e_int_menus_augmentation_find("enform/0");
   if (l) _e_int_menus_augmentation_add(subm, l);

   mi = e_menu_item_new(m);
   e_menu_item_label_set(mi, _("Enform"));
   e_util_menu_item_theme_icon_set(mi, "enform");
   e_menu_item_submenu_set(mi, subm);
   ...
   l = _e_int_menus_augmentation_find("enform/1");
   if (l) _e_int_menus_augmentation_add(subm, l);

   mi = e_menu_item_new(subm);
   e_menu_item_separator_set(mi, 1);

   l = _e_int_menus_augmentation_find("enform/2");
   if (l) _e_int_menus_augmentation_add(subm, l);
...
_e_int_menus_main_del_hook(void *obj)
{
   ...
   if (dat->enform) e_object_del(E_OBJECT(dat->enform));
   ...
   if (mi->icon_object)
     {
        ...
        Evas_Coord x, y, w, h;
        unsigned int size;
        const char *drag_types[] = { "enform/desktop" };
