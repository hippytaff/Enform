...
static const char *_type_enform_x_file = NULL;
...
e_dnd_init(void)
{
   ...
   _type_enform_x_file = eina_stringshare_add("enform/x-file");
...
e_dnd_shutdown(void)
{
   ...
   eina_stringshare_del(_type_enform_x_file);
   _type_text_uri_list = NULL;
   _type_xds = NULL;
   _type_text_x_moz_url = NULL;
   _type_enform_x_file = NULL;
   ...
   for (i = 0; i < ev->num_types; i++)
     {
        ...
        if (t)
                 ...
                 (!strcmp(_type_text_x_moz_url, ev->types[i])))  /* not sure it is stringshared */
          {
             ...
             EINA_LIST_FOREACH(_drop_handlers, l, h)
               {
                  h->active = (h->type == _type_enform_x_file);
                  h->entered = 0;
               }
             break;
          }
