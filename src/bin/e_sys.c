...
_e_sys_cb_timer(void *data __UNUSED__)
{
   ...
   snprintf(buf, sizeof(buf),
            "%s/enform/utils/enform_sys -t halt",
            e_prefix_lib_get());
   _e_sys_halt_check_exe = ecore_exe_run(buf, NULL);
   snprintf(buf, sizeof(buf),
            "%s/enform/utils/enform_sys -t reboot",
            e_prefix_lib_get());
   _e_sys_reboot_check_exe = ecore_exe_run(buf, NULL);
   snprintf(buf, sizeof(buf),
            "%s/enform/utils/enform_sys -t suspend",
            e_prefix_lib_get());
   _e_sys_suspend_check_exe = ecore_exe_run(buf, NULL);
   snprintf(buf, sizeof(buf),
            "%s/enform/utils/enform_sys -t hibernate",
            e_prefix_lib_get());
...
_e_sys_current_action(void)
{
   ...
   e_dialog_title_set(dia, _("Enform is busy with another request"));
   e_dialog_icon_set(dia, "enform/sys", 64);
...
//          return 0;
      ...
      case E_SYS_HALT_NOW:
        ...
        snprintf(buf, sizeof(buf),
                 "%s/enform/utils/enform_sys halt",
                 e_prefix_lib_get());
        if (_e_sys_exe)
          {
      ...
      case E_SYS_REBOOT:
        ...
        if (e_util_immortal_check()) return 0;
        snprintf(buf, sizeof(buf),
                 "%s/enform/utils/enform_sys reboot",
                 e_prefix_lib_get());
        if (_e_sys_exe)
          {
             if ((ecore_time_get() - _e_sys_begin_time) > 2.0)
      ...
      case E_SYS_SUSPEND:
        ...
        snprintf(buf, sizeof(buf),
                 "%s/enform/utils/enform_sys suspend",
                 e_prefix_lib_get());
        if (_e_sys_exe)
          {
             if ((ecore_time_get() - _e_sys_begin_time) > 2.0)
      ...
      case E_SYS_HIBERNATE:
        ...
        snprintf(buf, sizeof(buf),
                 "%s/enform/utils/enform_sys hibernate",
                 e_prefix_lib_get());
        if (_e_sys_exe)
          {
             if ((ecore_time_get() - _e_sys_begin_time) > 2.0)
