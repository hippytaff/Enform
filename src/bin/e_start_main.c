...
_env_path_prepend(const char *env, const char *path)
{
   ...
   // BOGO
   int total_len = len + 1 + len2 + 1;
   s = malloc(total_len);
   if (s)
     {
        s[0] = 0;
        snprintf(s, total_len, "%s%s", s, p2);
        //strcat(s, p2);
        if (p)
          {
             snprintf(s, total_len, "%s%s", s, ":");
             //strcat(s, ":");
             snprintf(s, total_len, "%s%s", s, p);
             //strcat(s, p);
          }
        env_set(env, s);
        free(s);
...
main(int argc, char **argv)
{
        ...
        if (!strcmp(argv[i], "-valgrind-gdb"))
                 ...
                 (!strcmp(argv[i], "--help")))
          {
             ...
             (
               "Options:\n"
               "\t-valgrind[=MODE]\n"
               "\t\tRun enform from inside valgrind, mode is OR of:\n"
               ...
               "\t\t all = all of above\n"
               "\t-massif\n"
               "\t\tRun enform from inside massif valgrind tool.\n"
               "\t-callgrind\n"
               "\t\tRun enform from inside callgrind valgrind tool.\n"
   ...
   /* run e directly now */
   snprintf(buf, sizeof(buf), "%s/enform", eina_prefix_bin_get(pfx));
                       ...
                       if ((WIFSTOPPED(status)) && (!stop_ptrace))
                         {
                            ...
                            snprintf(buffer, 4096,
                                     backtrace_str ?
                                     "%s/enform/utils/enform_alert %i %i '%s' %i" :
                                     "%s/enform/utils/enform_alert %i %i '%s' %i",
