   ...
   if (e_config)
     {
        ...
        if ((e_config->config_version) < E_CONFIG_FILE_EPOCH * 1000000)
          {
             ...
             ecore_timer_add(1.0, _e_config_cb_timer,
                             _("Settings data needed upgrading. Your old settings have<br>"
                               ...
                               "bug. This simply means Enform needs new settings<br>"
        ...
        /* config is too new? odd! suspect corruption? */
        else if (e_config->config_version > E_CONFIG_FILE_VERSION)
          {
             ...
             reload = 1;
             ecore_timer_add(1.0, _e_config_cb_timer,
                             _("Your settings are NEWER than Enform. This is very<br>"
                               "strange. This should not happen unless you downgraded<br>"
                               "Enform or copied the settings from a place where<br>"
                               "a newer version of Enform was running. This is bad and<br>"
        ...
        if (dia)
          {
             ...
             e_dialog_title_set(dia, _("Enform Settings Write Problems"));
             e_dialog_icon_set(dia, "dialog-error", 64);
             snprintf(buf, sizeof(buf),
                      _("Enform has had an error while moving config files<br>"
                        "from:<br>"
                        "%s<br>"
                        "<br>"
                        "to:<br>"
   ...
   switch (err)
     {
      ...
      default: /* if we get here eet added errors we don't know */
        erstr = _("The error is unknown to Enform.");
        break;
     }
   if (erstr)
     {
             ...
             if (dia)
               {
                  ...
                  e_dialog_icon_set(dia, "dialog-error", 64);
                  snprintf(buf, sizeof(buf),
                           _("Enform has had an error while writing<br>"
                             "its config file.<br>"
                             "%s<br>"
                             "<br>"
