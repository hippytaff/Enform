...
_e_fm_main_eeze_vol_mount_timeout(E_Volume *v)
{
   ...
   size = _e_fm_main_eeze_format_error_msg(&buf, v, "org.enform.fm2.MountTimeout", "Unable to mount the volume with specified time-out.");
...
_e_fm_main_eeze_vol_unmount_timeout(E_Volume *v)
{
   ...
   v->guard = NULL;
   eeze_disk_cancel(v->disk);
   size = _e_fm_main_eeze_format_error_msg(&buf, v, "org.enform.fm2.UnmountTimeout", "Unable to unmount the volume with specified time-out.");
                             ...
                             Eeze_Event_Disk_Error *ev)
{
   ...
   if (v->mounted)
     {
        size = _e_fm_main_eeze_format_error_msg(&buf, v, "org.enform.fm2.UnmountError", ev->message);
        ecore_ipc_server_send(_e_fm_ipc_server, 6 /*E_IPC_DOMAIN_FM*/, E_FM_OP_UNMOUNT_ERROR,
                              0, 0, 0, buf, size);
     }
   else
     {
        size = _e_fm_main_eeze_format_error_msg(&buf, v, "org.enform.fm2.MountError", ev->message);
        ecore_ipc_server_send(_e_fm_ipc_server, 6 /*E_IPC_DOMAIN_FM*/, E_FM_OP_MOUNT_ERROR,
                              0, 0, 0, buf, size);
     }
   free(buf);
...
_e_fm_main_eeze_vol_eject_timeout(E_Volume *v)
{
   ...
   v->guard = NULL;
   eeze_disk_cancel(v->disk);
   size = _e_fm_main_eeze_format_error_msg(&buf, v, "org.enform.fm2.EjectTimeout", "Unable to eject the media with specified time-out.");
   ...
   if (!eeze_disk_mount_wrapper_get(v->disk))
     {
        ...
        snprintf(buf, sizeof(buf), "%s/enform/utils/enform_sys", eina_prefix_lib_get(pfx));
...
_e_fm_main_eeze_volume_mount(E_Volume *v)
{
   ...
   if (!eeze_disk_mount_wrapper_get(v->disk))
     {
        ...
        snprintf(buf2, sizeof(buf2), "%s/enform/utils/enform_sys", eina_prefix_lib_get(pfx));
   ...
   return;
error:
   size = _e_fm_main_eeze_format_error_msg(&buf, v, "org.enform.fm2.MountError", "Critical error occurred during mounting!");
   ecore_ipc_server_send(_e_fm_ipc_server, 6 /*E_IPC_DOMAIN_FM*/, E_FM_OP_MOUNT_ERROR,
                         0, 0, 0, buf, size);
   free(buf);
