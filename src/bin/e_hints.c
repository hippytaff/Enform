...
EAPI Ecore_X_Atom ATM_ENFORM_COMMS = 0;
EAPI Ecore_X_Atom ATM_ENFORM_VERSION = 0;
EAPI Ecore_X_Atom ATM_ENFORM_SCALE = 0;
...
e_hints_init(void)
{
   ...
   ATM_ENFORM_COMMS = atoms[3];
   ATM_ENFORM_VERSION = atoms[4];
   ATM_ENFORM_SCALE = atoms[5];
                  ...
                  for (;; )
                    {
                       ...
                       if ((ecore_time_get() - ts) > 2.0)
                         {
                            e_error_message_show(_("A previous instance of Enform is still active\n"
...
/*	     ecore_x_netwm_wm_identify(roots[i], win, "KWin");*/
             ecore_x_netwm_wm_identify(roots[i], win, "Enform");
...
e_hints_e16_comms_pretend(E_Manager *man)
{
   ...
   snprintf(buf, sizeof(buf), "Enform %s", VERSION);
   ecore_x_window_prop_property_set(win, ATM_ENFORM_VERSION, ECORE_X_ATOM_STRING, 8, buf, strlen(buf));
   ecore_x_window_prop_property_set(man->root, ATM_ENFORM_VERSION, ECORE_X_ATOM_STRING, 8, buf, strlen(buf));

   snprintf(buf, sizeof(buf), "WINID %8x", (int)win);
   ecore_x_window_prop_property_set(win, ATM_ENFORM_COMMS, ECORE_X_ATOM_STRING, 8, buf, 14);

   ecore_x_window_prop_property_set(man->root, ATM_ENFORM_COMMS, ECORE_X_ATOM_STRING, 8, buf, 14);
}
   ...
   if (roots)
     {
        scale = e_scale * 1000;
        for (i = 0; i < num; i++)
          ecore_x_window_prop_card32_set(roots[i], ATM_ENFORM_SCALE, &scale, 1);
        free(roots);
     }
}
